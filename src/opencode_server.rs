...
mod tests {
    ...
    fn task_list_projects_only_the_calling_sessions_tasks() {
        ...
        let owner = session(workspace.path(), "list-owner");
        let other = session(workspace.path(), "list-other");
        ...
        let elsewhere = session(other_workspace.path(), "list-owner");
        let owner_task = Uuid::new_v4();
        store
            .save(&record_for(&owner, owner_task, TaskStatus::Running, None))
            .unwrap();
        ...
        assert_eq!(view["backend"], "opencode");
    ...
    fn task_list_counts_unreadable_records_as_skipped() {
        ...
        let store_root = tempfile::tempdir().unwrap();
        let store = TaskStore::new(store_root.path().join("tasks"));
        let owner = session(workspace.path(), "list-skipped");
        store
            .save(&record_for(
                &owner,
                Uuid::new_v4(),
    ...
    fn task_list_orders_newest_first_and_truncates_at_limit() {
        ...
        let store_root = tempfile::tempdir().unwrap();
        let store = TaskStore::new(store_root.path().join("tasks"));
        let owner = session(workspace.path(), "list-order");
        let now = config::unix_time();
        let ids: Vec<Uuid> = (0..3).map(|_| Uuid::new_v4()).collect();
        for (index, task_id) in ids.iter().enumerate() {
            let mut record = record_for(&owner, *task_id, TaskStatus::Running, None);
    ...
    fn task_list_reports_an_empty_store_as_empty() {
        ...
        let store = TaskStore::new(store_root.path().join("tasks"));
        let owner = session(workspace.path(), "list-empty");
    ...
    fn task_list_rejects_invalid_limits() {
        ...
        let store_root = tempfile::tempdir().unwrap();
        let store = TaskStore::new(store_root.path().join("tasks"));
        let owner = session(workspace.path(), "list-limit");
    ...
    fn task_view_separates_execution_from_verification_and_delivery() {
        ...
        let task_id = Uuid::new_v4();
        let mut record = record_for(&owner, task_id, TaskStatus::Completed, Some("oc-session-1"));
