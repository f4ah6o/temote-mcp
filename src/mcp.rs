...
fn tools(public: bool) -> Value {
    let mut tools = json!([
        ...
        {"name":"write_file","title":"Write a local file","description":"Write a UTF-8 file using the selected session permission mode. Normal sessions are restricted to permitted roots and use the local-mcp sandbox; yolo sessions may write anywhere the local user can.","annotations":{"readOnlyHint":false,"destructiveHint":true,"idempotentHint":true,"openWorldHint":false},"inputSchema":{"type":"object","properties":{"session_id":{"type":"string"},"path":{"type":"string"},"content":{"type":"string"}},"required":["session_id","path","content"],"additionalProperties":false}},
        ...
        {"name":"execute","title":"Run a command","description":"Execute argv without a shell using the selected session permission mode. Normal sessions run in the local-mcp sandbox with network disabled; yolo sessions run directly on the host with the local user's filesystem, environment, process, and network permissions. Returns the normal result when it finishes within 30 seconds; otherwise returns a job_id.","annotations":{"readOnlyHint":false,"destructiveHint":true,"idempotentHint":false,"openWorldHint":true},"inputSchema":{"type":"object","properties":{"session_id":{"type":"string"},"command":{"type":"array","items":{"type":"string"},"minItems":1},"cwd":{"type":"string"}},"required":["session_id","command"],"additionalProperties":false}},
