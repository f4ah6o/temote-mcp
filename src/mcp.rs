...
fn tools(_public: bool, managed_sessions: bool) -> Value {
    let mut tools = json!([
        ...
        {"name":"devin_cloud_task_start","title":"Start a Devin Cloud session task","description":"Accept an idempotent scoped Devin Cloud task mutation, persist acceptance before the remote side effect, then create a hosted Devin session (API v3) with a structured report schema. The session runs on Devin Cloud, not on this host; operation_id is mandatory.","annotations":{"readOnlyHint":false,"destructiveHint":true,"idempotentHint":true,"openWorldHint":true},"inputSchema":{"type":"object","properties":{"session_id":{"type":"string"},"operation_id":{"type":"string","format":"uuid"},"task":{"type":"string","minLength":1,"maxLength":1048576},"title":{"type":"string","minLength":1,"maxLength":256},"devin_mode":{"type":"string","description":"Devin session mode. The swe-2-medium/high/max values select SWE-2 reasoning effort only.","enum":["normal","fast","lite","ultra","fusion","swe-2-medium","swe-2-high","swe-2-max"]},"swe_tier":{"type":"string","description":"Optional SWE-2 service tier. promo keeps the requested SWE-2 mode and relies on upstream account promo eligibility; priority requires Temote to discover one exact account-visible priority/fast SWE-2 UID before remote session creation.","enum":["promo","priority"]},"repos":{"type":"array","items":{"type":"string","minLength":1,"maxLength":256},"maxItems":16},"max_acu_limit":{"type":"integer","minimum":1,"maximum":100000}},"required":["session_id","operation_id","task"],"additionalProperties":false}},
