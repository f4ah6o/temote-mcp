...
fn tools(_public: bool, managed_sessions: bool) -> Value {
    let mut tools = json!([
        ...
        {"name":"devin_task_start","title":"Start a scoped Devin task","description":"Accept an idempotent scoped Devin task mutation, persist acceptance before child side effects, then start a per-task devin acp child process over stdio and create its ACP session. operation_id is mandatory; no sandbox escape option is exposed. model, effort, and speed are applied through advertised ACP session configuration when available; speed is standard or priority. Set cloud=true to relay through `devin acp --cloud`; agent remains local-only.","annotations":{"readOnlyHint":false,"destructiveHint":true,"idempotentHint":true,"openWorldHint":true},"inputSchema":{"type":"object","properties":{"session_id":{"type":"string"},"operation_id":{"type":"string","format":"uuid"},"task":{"type":"string","minLength":1,"maxLength":1048576},"model":{"type":"string","minLength":1,"maxLength":256},"effort":{"type":"string","minLength":1,"maxLength":256},"speed":{"type":"string","enum":["standard","priority"]},"agent":{"type":"string","minLength":1,"maxLength":256},"cloud":{"type":"boolean"}},"required":["session_id","operation_id","task"],"additionalProperties":false}},
