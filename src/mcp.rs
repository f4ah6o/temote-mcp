...
fn tools(public: bool, managed_sessions: bool) -> Value {
    let mut tools = json!([
        ...
        {"name":"write_file","title":"Write a local file","description":"Write a UTF-8 file using the selected session permission mode. Normal sessions are restricted to permitted roots and use the temote-mcp sandbox; yolo sessions may write anywhere the local user can.","annotations":{"readOnlyHint":false,"destructiveHint":true,"idempotentHint":true,"openWorldHint":false},"inputSchema":{"type":"object","properties":{"session_id":{"type":"string"},"path":{"type":"string"},"content":{"type":"string"}},"required":["session_id","path","content"],"additionalProperties":false}},
