...
fn validate_initialize_response(value: &Value) -> Result<()> {
    ...
    anyhow::ensure!(
        app_server_version_from_user_agent(user_agent)
            .is_some_and(|version| version == SUPPORTED_APP_SERVER_VERSION),
...
"##
    ...
    #[test]
    fn initialize_validation_accepts_real_0_153_4_user_agent_shapes() {
        for user_agent in [
            format!(
            ...
            assert_eq!(
                app_server_version_from_user_agent(&user_agent),
                Some(SUPPORTED_APP_SERVER_VERSION)
            );
        }
    }
    ...
    fn initialize_validation_rejects_wrong_or_ambiguous_user_agents() {
        let invalid = [
            ...
            format!(
                "temote-mcp/0.153.5 (Ubuntu 24.4.0; x86_64) unknown (temote-mcp; {APP_SERVER_CLIENT_VERSION})"
            ),
            format!(
    ...
    #[test]
    fn initialize_validation_requires_the_0_153_4_response_shape() {
