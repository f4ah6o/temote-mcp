...
#[cfg(feature = "network")]
fn validate_swe_tier(value: Option<&str>, devin_mode: Option<&str>) -> Result<()> {
    let Some(value) = value else {
        return Ok(());
    };
    anyhow::ensure!(
        matches!(value, "promo" | "priority"),
        "swe_tier must be one of promo, priority"
    );
    anyhow::ensure!(
        matches!(
            devin_mode,
            Some("swe-2-medium" | "swe-2-high" | "swe-2-max")
        ),
...
mod tests {
    ...
    fn task_start_keeps_devin_cloud_options() {
        ...
        let cases = [
            ...
            (
                json!({"devin_mode": "fast", "swe_tier": "promo"}),
                "swe_tier requires devin_mode to be one of swe-2-medium, swe-2-high, swe-2-max",
            ),
            (
