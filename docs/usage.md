...
The optional `cwd` is canonicalized and must remain inside a permitted session root, including after symlink resolution, even when the session is yolo. The `read_only` and `workspace_write` modes select the corresponding agent policy. Temote's outer agent profile makes only the selected canonical `cwd` writable for `workspace_write`; the other permitted roots remain read-only, while private per-run state/cache remains writable in either mode. `.git`, `.agents`, and `.codex` remain protected.
