...
An `Applied` resume receipt acknowledges reconciliation, not successful continuation or task completion. Inspect the reconciled task state: the 2026-09-15 stopped-child qualification returned `Applied` with `Interrupted`; it did not directly record a replacement-process PID or demonstrate completion after resume. See the [residual validation evidence](evaluations/completion-residual-validation-20260915.md).
