...
Typed `resume` reconciles the retained thread and turn; it does not start a new turn or revive a terminated child process. An `Applied` resume receipt acknowledges reconciliation, not successful continuation or task completion. Inspect the reconciled task state: the 2026-09-15 stopped-child qualification returned `Applied` with `Interrupted`; it did not directly record a replacement-process PID or demonstrate completion after resume. See the [evaluation results](evaluations/completion-evaluation-results-20260915.md#current-residual-disposition-2026-09-15).
