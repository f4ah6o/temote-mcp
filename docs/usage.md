...
- `agent` is the default for newly created sessions, including authenticated public `session_start`. It keeps the same sandbox, path containment, and tool-specific validation, and its ordinary `execute`/`start_command` runs use the network-enabled development sandbox profile, but it does not require the local approval console for otherwise-valid structured operations: Git fetch/pull/branch-push/tag-push, `local_agent_run`, `dev_tool_run`, checkpoints, patches, the structured 1Password/kintone integrations, and the Codex app-server / OpenCode serve / Devin ACP delegation tools (`codex_status`, `codex_task_*`, `opencode_status`, `opencode_task_*`, `devin_status`, `devin_task_*`).
