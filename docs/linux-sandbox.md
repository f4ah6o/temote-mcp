...
`temote-linux-sandbox` always enters bubblewrap before the requested command executes. The ordinary sandbox uses a read-only bind of `/`, explicit writable binds for the working directory and approved roots, `/tmp` and `TMPDIR` according to Temote's existing semantics, user/PID/network namespaces, `--die-with-parent`, and `--new-session`. Ordinary commands retain their existing top-level `.git`, `.agents`, and `.codex` masks; they do not recursively walk writable roots. The network-enabled development profile used by `agent`-mode ordinary commands and structured developer-tool operations keeps the same workspace/state containment; the operation class selects whether outbound network is enabled or denied. Validated Git operations can write the repository metadata needed for add/commit while configuration, hooks, unrelated refs, and other protected metadata remain read-only.

The helper compiles Temote's network/process seccomp policy with `seccompiler`, writes the compiled cBPF program to a sealed anonymous memfd, and passes that fd to bubblewrap using `--seccomp FD`. The network-enabled development profile denies path-based `AF_UNIX` sockets while permitting only unnamed `AF_UNIX` socketpairs with `SOCK_STREAM|SOCK_CLOEXEC|SOCK_NONBLOCK` and protocol 0, which is the form needed by runtimes such as Tokio for in-process signal handling; datagram pairs, other socket types, and nonzero protocols remain denied, and the profile still permits outbound network sockets. There is no hidden self-dispatch stage that can execute a command outside bubblewrap. Bubblewrap establishes `no_new_privs` before applying the seccomp filter to the sandbox child.
