...
`temote-linux-sandbox` always enters bubblewrap before the requested command executes. The ordinary sandbox uses a read-only bind of `/`, explicit writable binds for the working directory and approved roots, `/tmp` and `TMPDIR` according to Temote's existing semantics, user/PID/network namespaces, `--die-with-parent`, and `--new-session`. The local-agent profile hides the host temporary and user-agent state roots, re-exposes only the executable directories, selected workspace roots, and private run state it needs, keeps workspace roots writable only for `workspace_write`, and enables outbound network access without exposing a generic policy selector through MCP. Protected metadata such as `.git`, `.agents`, and `.codex` remains read-only. Validated Git operations can write the repository metadata needed for add/commit while configuration, hooks, unrelated refs, and other protected metadata remain read-only.
