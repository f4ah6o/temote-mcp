...
For an application that must resolve additional reviewed references after startup, pass the exact references in `allowed_locators`. On Linux, Temote resolves every allowed locator before the target starts and stores only that exact locator-to-value map in a per-invocation Unix-domain-socket broker. The token-bearing `op read` processes therefore finish before the untrusted target exists. Temote also serializes all raw-token CLI work against active service-account targets, so a concurrent invocation cannot introduce a token-bearing sibling process while a target is running. The child receives only `TEMOTE_MCP_SECRET_RESOLVER_SOCKET` and `TEMOTE_MCP_SECRET_RESOLVER_TOKEN`; broker authorization additionally requires the connecting PID to be the bound target process or one of its descendants, so a same-UID process cannot reuse a stolen capability. The broker never performs additional 1Password reads after target startup and is destroyed when the command exits. On macOS, requesting `allowed_locators` currently fails closed; ordinary non-nested service-account execution is unchanged.
