...
The session process holds the token in memory; it is not written to session JSON or returned by MCP tools. On Linux, Temote disables dumpability before parsing commands whenever `OP_SERVICE_ACCOUNT_TOKEN` is present, so same-UID descendants cannot recover the supervisor startup environment through `/proc` or ptrace-style inspection.

- `onepassword_service_account_status` checks token presence and `op whoami` without returning the token.
- `onepassword_service_account_run` resolves reviewed `op://` inputs through 1Password before launching the target command.

Provide secret values as `op://...` references through `environment`, or use checked-in env templates through `env_files`. Plaintext secret values in `environment` are rejected. For env files, only assignments whose value contains an `op://` reference are treated as secret-bearing for output redaction; unsupported assignment syntax fails closed. Use this direct substitution path when all required secrets are known before the command starts. Temote completes the token-bearing `op run` substitution into supervisor memory first, waits for that CLI process to exit, then launches the target directly without `OP_SERVICE_ACCOUNT_TOKEN`. Resolved values are redacted from captured target output.

For an application that must resolve additional reviewed references after startup, pass the exact references in `allowed_locators`. On Linux, Temote resolves every allowed locator before the target starts and stores only that exact locator-to-value map in a per-invocation Unix-domain-socket broker. The token-bearing `op read` processes therefore finish before the untrusted target exists. The supervisor disables peer process inspection before handling requests when a service-account credential is present, and supervisor upgrade/re-exec transfers that credential through a sealed mode-000 anonymous FD rather than placing the raw token in the new startup environment. Raw-token CLI execution is allowed only when the resolved `op` executable has a kernel-enforced process-inspection boundary: it must be a root-owned, non-writable setgid executable whose setgid group is not available to the Temote user, and Linux `fs.suid_dumpable=1` is rejected. Service-account targets run in a private PID namespace with a private `/proc`, so they cannot inspect the supervisor or token-bearing processes from this or another Temote supervisor. The child receives only `TEMOTE_MCP_SECRET_RESOLVER_SOCKET` and `TEMOTE_MCP_SECRET_RESOLVER_TOKEN`; broker authorization additionally requires the connecting PID to be the bound target process or one of its descendants, so a same-UID process cannot reuse a stolen capability. The broker never performs additional 1Password reads after target startup and is destroyed when the command exits. Long-running targets do not lock out unrelated service-account status or resolution work. On macOS, requesting `allowed_locators` currently fails closed; ordinary non-nested service-account execution is unchanged.
