...
`just up` keeps `temote-mcp serve` as the foreground process and lets it own `cloudflared` as a child. This gives the approval console the terminal stdin and ties Tunnel cleanup to the same shutdown path.
