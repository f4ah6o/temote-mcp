...
resume の receipt が `Applied` でも、実行の再開成功や task の完了を意味しません。reconcile 後の task state を確認してください。2026-09-15 の終了済み child を対象とした検証結果は `Applied` と `Interrupted` の組合せであり、replacement process PID の直接記録と resume 後の完了確認はありません。[残存検証の記録](evaluations/completion-residual-validation-20260915.md)を参照してください。
