...
上の `devin acp` tool は *local* の Devin CLI を駆動します。これとは別の `devin_cloud_status`、`devin_cloud_task_start`、`devin_cloud_task_get`、`devin_cloud_task_control` は、Temote host から HTTPS で Devin API v3 (`https://api.devin.ai/v3/organizations/{org_id}/sessions`) を呼び、*hosted* な Devin session を駆動します。作業は Temote session の working directory ではなく Devin Cloud 側で行われ、local Devin binary が必要なのは明示的な SWE-2 priority discovery の場合だけです。これらの tool は `network` feature build にのみ存在します。通常の hosted task では Devin binary は不要ですが、SWE-2 で明示的に `swe_tier: "priority"` を指定した場合だけ、account の model catalog を read-only preflight するため local Devin CLI を使います。
