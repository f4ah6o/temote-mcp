...
型付き `resume` は保持済みのthread / turnをreconcileする操作であり、新しいturnの開始や終了済みchild processの再起動は行いません。receiptが `Applied` でも実行再開やtask完了の証明ではないため、reconcile後のtask stateを確認してください。2026-09-15の停止後検証は `Applied + Interrupted` で、replacement process PIDの直接記録とresume後の完了確認はありません。[評価結果](evaluations/completion-evaluation-results-20260915.md#current-residual-disposition-2026-09-15)を参照してください。
