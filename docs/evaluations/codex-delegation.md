...
Historical static verification only (not live evidence): at this point in the 2026-09-13 evaluation, `src/codex_app_server.rs` still pinned the initialize handshake to a specific app-server version. That policy was later removed: current Temote treats the parsed version as diagnostic metadata only and validates the bounded initialize shape plus the concrete `model/list`, `thread/*`, and `turn/*` contracts it actually consumes. The rest of this historical observation remains applicable: `codex_status` issues `model/list` with `includeHidden:true`, `codex_task_start` validates the requested model/effort against the advertised list before `thread/start`, control is limited to typed `steer`/`resume`/`interrupt`, and the four tools are registered in `src/mcp.rs`. A follow-up schema review below found and fixed a real protocol mismatch.
