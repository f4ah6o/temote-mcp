...
`just up` は `temote-mcp serve` を foreground process にし、`cloudflared` を child として管理します。これにより approval console が端末 stdin を所有し、Tunnel cleanup も同じ shutdown path に結合されます。
