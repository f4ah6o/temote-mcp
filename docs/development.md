...
On Linux, build or install the sibling `temote-linux-sandbox` binary and install a root-owned, non-writable `bwrap` at `/usr/bin/bwrap` (or the canonical `/bin/bwrap` path). The sandbox does not resolve bubblewrap from the child-controlled `PATH`. macOS uses the system Seatbelt sandbox. Native Windows is not supported. See [Linux sandbox and crates.io packaging](linux-sandbox.md) for the helper boundary and acceptance tests.
...
Repository tests keep the chain honest: the Rust snapshot tests regenerate and compare both checked-in files, the gateway parity test compares `PUBLIC_TOOLS` plus its own computed fingerprint against those same files, and the `Allocate Release` workflow runs an explicit `Verify public contract parity` step before publishing. Source presence does not imply connected runtime availability: after a deploy, upgrade, or session restart, read the fingerprint from the actual connected runtime and compare it with the repository value instead of assuming a merged commit or a restarted session refreshed the connector schema. When a tool is missing from the connected surface, fix or redeploy the runtime instead of falling back to yolo mode or other workarounds.
...
The server-backed task tools are the primary delegation interfaces: `codex_task_*` drives the installed Codex through `codex app-server --stdio`, and `opencode_task_*` drives it through `opencode serve` + the `unofficial-opencode-sdk` client. The `temote-mcp delegate` and `temote-mcp codex delegate` commands, which invoke the installed `codex exec` / `opencode run` argv contracts, are the legacy one-shot fallback and may be removed once server parity is fully demonstrated. The `local_agent_run` broker is a separate per-command sandbox tier and is not covered by that fallback designation.
...
This bootstrap is developer infrastructure, not a generic remote shell. It does not widen filesystem roots, bypass approvals, or alter the session network boundary. It remains available as the legacy CLI fallback described above.

The experimental MCP surfaces `codex_status`, `codex_task_start`, `codex_task_get`, and `codex_task_control` use the local `codex app-server --stdio` protocol. Compatibility is not pinned to a specific app-server version string. The initialize response must have the bounded fields Temote needs (including an absolute Codex home); the version parsed from `userAgent` is diagnostic only. The adapter then validates the concrete `model/list`, `thread/*`, and `turn/*` response shapes it consumes, so an incompatible protocol fails at the actual contract boundary rather than at a release-number allowlist. Task records are bound to the complete Temote session instance (`session_id`, start time, and process ID) plus the canonical session directory. Store updates are serialized across Temote processes, and each live task runtime holds an owner-only OS lease until its child shuts down. A second process returns the persisted task view instead of reconstructing a runtime that is still owned elsewhere, marks that view `reconciliation_deferred`, rejects typed control before persistence, and does not finalize or prune the remotely leased task. Every mutation requires an opaque `operation_id`; its accepted receipt is persisted before spawning or sending a child request. Pre-thread initialization/model-list failures are retryable with the same start operation, while a failure after thread/start or turn/start may have been sent becomes `reconciliation_required` and is never replayed blindly. Control is limited to typed `steer`, `resume`, and `interrupt` actions, and accepted operation IDs remain conflict-protected through task retention even when detailed receipts are compacted.
...
Generated Codex turns are requested with `workspaceWrite`, the canonical session directory as the writable root, and `networkAccess=false`. This is an app-server sandbox adapter, not a replacement for Temote's OS-level session sandbox: the app-server process itself must communicate with the inference service outside that boundary. Keep these MCP surfaces opt-in/experimental until the host's installed Codex build and sandbox behavior have been validated; do not treat them as equivalent to Temote's session sandbox.

The experimental `opencode_*` surfaces mirror that contract over `opencode serve`: each task spawns a dedicated serve child on 127.0.0.1 with a dynamic port, a per-instance Basic-auth password injected through the child environment only, an isolated per-task `XDG_DATA_HOME` (which both isolates state and gives cross-process resume the same opencode session storage), and a bounded permission configuration passed through `OPENCODE_CONFIG_CONTENT`. Temote drives it with the `unofficial-opencode-sdk` client over the stable v1 API (`session create`/`prompt_async`/`abort`/`status`/`messages`, `permission`/`question` lists); the deterministic prompt `messageID` lets reconciliation distinguish a never-admitted start prompt from an uncertain post-prompt crash window. The module is compiled only under the `network` feature; all other wiring (supervisor session-shutdown hooks, approvals, activity coverage, the MCP dispatch table) is gated to match so `--no-default-features` keeps building.

The structured `local_agent_run` broker is a separate adapter for one-shot Codex and OpenCode work. Its Rust side resolves only an absolute installed executable, preserves the selected PATH candidate at runtime while validating its canonical target, constructs a broker-owned strict Codex permission-profile invocation or `opencode run --pure --format json --dir ...`, and transports Codex tasks through stdin with the verified `codex exec ... -` contract. OpenCode tasks remain bounded positional messages because the installed `opencode run [message..]` help contract does not verify stdin prompt transport. These contracts were checked with the installed `codex --help`, `codex exec --help`, `opencode --help`, and `opencode run --help` commands. It clears the child environment, imports only bounded standard auth files into read-only private run state, keeps imported auth usable by the top-level runtime while denying it to generated Codex commands/OpenCode tools, gives the child an explicit network-enabled Temote filesystem profile, re-exposes only the selected canonical cwd and the bounded executable path graph needed by its verified symlinks, exact launcher metadata files, and empty scaffolds for verified normal directories encountered before a `..`, enforces canonical session-root scope and recursively protected metadata rules, and uses the explicit user-approval path before every launch, including yolo sessions. This broker is intentionally limited to `local_agent_run`; it is not a Cargo or Vite+ broker, and it does not provide Git remote mutation.
