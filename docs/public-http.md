...
Preflight also reports `helper_generation`: a bounded `compatible` / `incompatible` / `unavailable` classification of the Linux sandbox helper bundled next to the installed executable. Apply refuses the handoff before any destructive step when the classification is not `compatible`. On platforms without a sandbox helper the classification is `compatible`.
...
Preflight also embeds the observed direct-ingress plan as `direct_ingress`, including bounded non-secret diagnostics: `runtime_root` (which derivation produced the runtime directory — `temote-runtime-dir`, `xdg-runtime-dir`, or `home-cache`), `runtime_root_id` (a truncated SHA-256 of the resolved directory that distinguishes same/different roots without revealing the path), `pid`, `host_id`, and `state_schema`. Two callers that classify the same ingress differently — for example a Temote-invoked versus a local-shell `upgrade --dry-run` observing different runtime directories — can compare these fields to identify the observation divergence instead of silently disagreeing.
