...
test("gateway routed tools and protocol versions match the Rust contract", () => {
  ...
  for (const name of ["git_branch_create", "git_switch", "git_worktree_add", "git_worktree_create", "git_worktree_list", "git_worktree_remove"]) {
...
test("the single MCP endpoint publishes the gateway tool list", async () => {
  ...
  assert.equal(response.status, 200);
  const rpc = await response.json();
  assert.equal(rpc.result.tools.length, 57);
  for (const required of ["host_list", "host_info", "session_start", "session_stop", "session_restart"]) {
