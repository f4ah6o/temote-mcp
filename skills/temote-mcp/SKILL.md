...
Typed `resume` reconciles the retained thread and turn; it does not start a new turn or revive a terminated child process. An `Applied` receipt is not proof of resumed execution or a completed task. Inspect the reconciled task state. The 2026-09-15 stopped-child qualification returned `Applied` with `Interrupted`; a replacement-process PID was not directly recorded, and completion after resume was not demonstrated. Preserve these limitations when reporting evidence; see `docs/evaluations/completion-evaluation-results-20260915.md`.
