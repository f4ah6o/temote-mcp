...
- Remote deletion performs one network approval first, then resolves the live symbolic remote `HEAD` and protection authority immediately before the lease-protected delete. GitHub branch metadata is used for GitHub destinations; non-GitHub remotes require the repository-local protection policy.
