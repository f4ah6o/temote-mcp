...
| [x] | S08 / S01,S07 | activity/render.rs の pure renderer と session_control.rs の finite replay client を作る。内部 test client から no-follow を検証する。 | `cargo test --lib activity::render` は3 tests、`cargo test --bin temote-mcp activity_client` は5 tests passed。固定時刻文字列、UUID短縮、control character、end 前 EOF を確認。まだ未完成の CLI は公開していない。 |
| [x] | S09 / S08 | CLI の follow/output adapter、cli.rs、main.rs の activity dispatch を追加する。 | `cargo test --bin temote-mcp activity_cli`。tail parser、TTY/non-TTY、BrokenPipe、Ctrl-C、5秒出力待ち。fixture broker で既定 follow と no-follow が動く。 |
| [x] | S10 / S06,S09 | mcp.rs の read_file/write_file、supervisor の session_start/session_stop に scope を接続し、最初の実操作 end-to-end 経路を完成させる。 | `cargo test --test activity_e2e activity_file_roundtrip`。一時 supervisor と2 sessionで read/write、filter、viewer 切断後の生存を確認する。 |
| [x] | S11 / S10 | approval helper の互換 wrapper と git_* の計測だけを追加する。 | `cargo test --bin temote-mcp activity_approval`。ask の allow/deny、不在 console、agent の skip。local bare remote で git_pull の state 順序と既存結果を確認。 |
| [x] | S12 / S11 | mcp.rs の Job/JobCompletion、spawn_sandboxed_command、execute/start_command/store_job/stop_job に同一 scope を接続する。 | `cargo test --bin temote-mcp activity_job`。foreground、job返却、stop/自然完了の両勝順、session stop、timeout。後述の競合手順を使う。 |
| [x] | S13 / S12 | spawn_local_agent / spawn_dev_tool に S12 の scope/result 確定手順を適用する。 | `cargo test --bin temote-mcp activity_delegated_job`。既存 fake executable で正常/deny/timeout/stop、prompt/output sentinel。sandbox と起動 argv は変更しない。 |
| [x] | S14 / S11,S13 | 未計測の同期 tool と integration を coverage 表どおり追加する。1Password、kintone、Codex は外側の call だけ。 | `cargo test --bin temote-mcp activity_coverage`。一覧の各 tool に対象/除外/owner/fixture を対応付ける。Codex accepted と turn completion を区別する。 |
| [x] | S15 / S05,S10 | 残る permission/restart/crash/auto-restart/forget/upgrade の lifecycle 計測を追加する。 | `cargo test --bin temote-mcp activity_lifecycle` と既存 upgrade tests。restart 二重発行なし、別 instance、exec後の履歴消失。upgrade成功の旧 terminal を補完しない。 |
...
- 2026-09-15: S08: done。Changed は `src/activity/contract.rs`（public strict `decode_event` と event envelope visitor）、`src/activity/render.rs`（pure bounded one-line renderer）、`src/activity.rs`（render module 登録）、`src/session_control.rs`（内部 finite no-follow replay client と activity 固有 strict attach response decoder）、本イシュー記録。`decode_event` は2048 bytes 上限を parse 前に確認し、envelope/event の未知・重複・欠落 field、nullable key の欠落、固定 enum、UUID、session identity、duration/state、summary/control、schema/sequence を checked decode する。serde error 入力は公開せず、構文は `invalid_json`、未知 schema 等は既存の固定 semantic `ContractError` を維持する。renderer は検証済み event と caller supplied の表示時刻文字列だけから、sequence、session、instance/op UUID 先頭8桁、operation/state、terminal duration、safe summary を含む1行を返し、clock/timezone/TTY/stdout/socket/env に触れない。時刻文字列は64 bytes以下で C0/C1、ESC、改行/CR、bidi control を拒否する。
  内部 `activity_replay()` は connect、request write、attach response、各 replay frame を各5秒で制限し、tail 0..=1024、control protocol 2、activity schema 1、generation、replayed count、strict sequence/filter/snapshot boundary と exact `activity_end` を検査する。S07 の postrequest EOF 契約に合わせ、end 受信まで socket write half を保持する。成功/失敗 response は raw JSON から activity 固有 map visitor へ直接 decode し、top-level/nested duplicate・unknown・missing key を Value 化前に拒否する。peer supplied error は診断へ出さず固定 `activity attachment rejected` にする。legacy `ControlResponse` / `ensure_response_ok` と既存 control client は変更していない。CLI parser/dispatch、local timezone 変換、follow/stdout adapter は S09 に残した。
  `cargo test --lib activity::contract` は18 passed / 0 failed / 87 filtered、`cargo test --lib activity::render` は3 passed / 0 failed / 102 filtered、`cargo test --bin temote-mcp activity_client -- --test-threads=1` は5 passed / 0 failed / 749 filtered、S07 attachment 回帰9件も成功した。decoder は round-trip、duplicate/unknown/missing nullable、semantic class、oversize、renderer は固定 golden/placeholder/control+bidi、client は real handler no-follow/write-half、end 前 EOF、schema mismatch、strict event、duplicate handshake metadata、secret/ESC peer error、success の missing error、failure の missing result を確認する。`cargo fmt --all -- --check`、`cargo clippy --all-targets -- -D warnings`、`cargo check --no-default-features --all-targets`、`git diff --check` は成功し、no-default の5 warning は従来箇所だけである。`gpt-6-astra` は初回 `ee7db4700432b2cca24cce19a7f2f5ba04403c9229d9180cb91964f7e8b59d5e` に activity response の Value duplicate collapse/raw error echo、次の `93dcb93493a422d924634e7ebad67cb6e5687394d449bc48cde5b6b4cec557f3` に serde nullable wrapper の missing-key 受理を指摘した。explicit presence visitor と回帰を含む final source diff `677505e6d89f4d5b5ba93bdffccf55928b0dd0c9f412d992bb34e206c226774f` を approve、全指摘 resolved。Interface for next step は `activity::contract::decode_event`、`activity::render::render_event`、内部 `ActivityReplay` / `activity_replay` と strict response/frame decoder。CHANGES.md impact は no（S08 は未公開 client/rendering primitives で、利用者向け記録は S16）。Remaining は S09〜S16、CLI follow/stdout/local time、実操作 coverage、lifecycle、docs である。
- 2026-09-15: S09: done。Changed は `src/cli.rs`（root `activity [SESSION_ID] [--tail N] [--no-follow]` parser/help。tail 既定100、範囲0..=1024、完全一致 session ID 検証）、`src/main.rs`（activity 専用 dispatch と bounded failure exit）、`src/session_control.rs`（strict replay/live client state、local timezone renderer adapter、TTY stdin/Ctrl-C/socket EOF、bounded stdout/stderr worker）、`tests/activity_cli_e2e.rs`（private fake supervisor と実 binary の PTY/pipe fixture）、本イシュー記録。CLI は supervisor を自動起動・再接続せず、no-follow は stdin を読まず `activity_end` で0、end 前 EOF は1、follow は replay 後の idle read に期限を設けず Ctrl-C・TTY EOF・socket EOF で viewer だけを0終了する。非TTY stdin は EOF でも監視しない。全 session live sequence は明示 gap 以外で連続を要求し、filtered stream は除外 event による sequence 飛びを許す。history truncation と global gap は現在状態を保証しない診断として表示する。
  stdout 256件、stderr 64件の queue は `try_send` のみで、各専用 OS thread と watchdog が1行5秒を上限にする。blocking `write(2)` 自体から戻らない場合も watchdog が async 側へ失敗を通知し、Ctrl-C 後の combined drain は5秒以内で打ち切る。stdin monitor はTTYだけを poll し、EOFとI/O失敗を区別し、Tokio blocking taskを残さない。dupした標準FDの file status flags は変更しない。activity dispatch の最終 error は100ms bounded writer後に明示 exit1とし、Rust `Result` の同期 stderr fallbackを通さない。stdout BrokenPipe は replay end 前でも0である。
  private `HOME` / `CODEX_HOME` / `XDG_STATE_HOME` / `XDG_RUNTIME_DIR` / `TEMOTE_MCP_RUNTIME_DIR` / `TMPDIR` と12文字以下の `TEMOTE_MCP_SOCKET_NAMESPACE` を使い、spawn child は `env_clear` した検証で、`cargo test --bin temote-mcp activity_cli -- --test-threads=1` は14 passed / 0 failed / 749 filtered、`cargo test --test activity_cli_e2e -- --test-threads=1` は7 passed / 0 failed。実 binary fixture は no-follow stdin EOF/end、end前EOF、5秒超のfollow idle、非TTY EOF、Ctrl-C、PTY EOF、follow socket EOF、BrokenPipe、no-follow drain中Ctrl-C、full stdout/stderr の5秒以内終了、blocked stderrでの最終error非blocking、history truncation表示を確認した。`cargo fmt --all -- --check`、`cargo clippy --all-targets -- -D warnings`、`cargo check --no-default-features --all-targets`、`git diff --check` は成功し、no-defaultの5件のdead-code warningは従来箇所に限られる。全suiteとgatewayはS16で実施する。Interface for next step は利用者向け `run_activity_command` と内部 strict `ActivityClientConnection` / `ActivityStreamState`、既存 `activity_replay()` である。CHANGES.md impact は no（利用者向け記録はS16でまとめる）。Remaining は S10〜S16 のoperation coverage、approval/job/lifecycle、docs、全gate、macOS実機確認である。
- 2026-09-15: S09 independent review: approved。`gpt-6-astra` は base `c6e78de` に対する immutable commit `d5021a5`、full diff SHA-256 `52dc50f545cfeabff095a0e4c1bfabf4e4da25bc6c04bfbecf627adb9c155578` を3 passで確認し、残る指摘なしで approve した。
- 2026-09-15: S10: done。Changed は `src/mcp.rs`（`read_file` / `write_file` の scope と write 実作業直前の `running`）、`src/supervisor.rs`（supervisor 所有の `session_start` / `session_stop` scope と broker emitter）、`tests/activity_e2e.rs`（private real supervisor、2 session、MCP、activity CLI の process fixture）、本イシュー記録。read は短時間操作として `started` から直接 terminal、write は入力・path 検証後かつ sandbox/yolo write 前に `running` へ進む。成功は `completed`、失敗は raw error を含めず固定 `operation_failed` の `failed` とし、activity の配送失敗は既存 MCP/lifecycle 結果へ影響させない。lifecycle は supervisor の public/local start と stop の所有箇所だけで発行し、MCP dispatch で重複発行しない。自動 restart 内部の `start_resolved` は S15 の外側 restart scope に残した。
  private `HOME` / `CODEX_HOME` / `XDG_STATE_HOME` / `XDG_RUNTIME_DIR` / `TEMOTE_MCP_RUNTIME_DIR` / `TMPDIR` と12文字以下の `TEMOTE_MCP_SOCKET_NAMESPACE` を使い、real child は `env_clear` して検証した。fixture の MCP response は nonblocking FD から10秒以内・8 MiB以内で読み、短命 CLI は stdout/stderr を並行 drain する child guard で10秒以内に終了させる。`cargo test --bin temote-mcp activity_lifecycle_start_and_stop_are_emitted_only_by_supervisor -- --test-threads=1` は1 passed / 0 failed / 763 filtered、成功 start、重複 start failure、stop の各1 scope・同一 operation ID pair・terminal 一回性を確認した。`cargo test --test activity_e2e activity_file_roundtrip -- --test-threads=1` は1 passed / 0 failedで、2 session の read/write、全 session replay、完全一致 filter、viewer 切断後に追加された read activity、viewer 切断後の MCP/runtime 生存、stop terminal を実 binary で確認した。`cargo fmt --all -- --check`、`cargo clippy --all-targets -- -D warnings`、`cargo check --no-default-features --all-targets`、`git diff --check` は成功し、no-default の5件の dead-code warning は従来箇所に限られる。全 suite と gateway は S16 で実施する。Interface for next step は `file_activity_scope` / `finish_file_activity` の best-effort terminal pattern、supervisor の `activity_scope` / `finish_supervisor_activity`、実 process fixture の `Fixture` / `McpClient` 構成である。CHANGES.md impact は no（operation coverage の途中であり、利用者向け記録は S16 でまとめる）。Remaining は S11〜S16 の approval、git、job、delegated/integration tool、残る lifecycle、docs、全 gate、macOS 実機確認である。
- 2026-09-15: S10 independent review: approved。`gpt-6-astra` は base `d5021a5` に対する immutable head `c29eac8`、full binary diff SHA-256 `d68a7a6a7e37367bdac29e0c5ff0bfe4db86654d53a420104763b049d09d7a1d` を3 passで確認した。初回 snapshot の fixture にあった無期限 `read_line` / `Command::output` を nonblocking bounded read と child guard に修正後、残る指摘なしで approve した。
- 2026-09-15: S11: done。Changed は `src/approvals.rs`（既存引数と戻り値を保つ `ensure_local_approval` wrapper、optional scope を受ける `ensure_local_approval_with_activity`）、`src/activity/contract.rs` / `src/activity/render.rs`（`git_add` / `git_commit` / `git_fetch` / `git_push` operation の追加と固定表示）、`src/mcp.rs`（5つの Git tool scope、safe remote 分類、approval/running/terminal 接続と fixture）、本イシュー記録。既存 `local_approval(mode, class)` の policy は変更せず、実際に `Request` / `RequestUser` を送る直前だけ `waiting_approval` を試行する。Skip は waiting を発行せず、allow 後は実 Git 処理直前に `running`、明示 deny は固定 `approval_denied`、console 不在などの approval error は raw error を activity に入れず `operation_failed` で terminal にする。既存の Git validation、sandbox、ff-only、remote/refspec 制限、approval return value、tool response は維持した。明示 remote は user supplied 名を保持せず `origin` / `other` の固定分類だけを出し、Git 設定が選ぶ暗黙の pull/push remote は誤推測せず空 summary とする。`push --set-upstream` の省略 remote は既存 argv と同じ `origin` に分類する。
  private outer test environment で `cargo test --bin temote-mcp activity_approval -- --test-threads=1` は4 passed / 0 failed / 774 filtered。local bare remote と実 Git で ask allow の `started → waiting_approval → running → completed`、deny の `started → waiting_approval → failed`、console 不在の fail-closed、agent の `started → running → completed`、`git_add` / `git_commit` / `git_fetch` / `git_pull` / `git_push` の terminal 一回性と既存成功 result、任意 remote sentinel が `remote=other` だけになることを確認した。`cargo test --lib activity::contract -- --test-threads=1` は18 passed / 0 failed / 88 filtered、`cargo test --lib activity::render -- --test-threads=1` は3 passed / 0 failed / 103 filtered。`cargo fmt --all -- --check`、`cargo clippy --all-targets -- -D warnings`、`cargo check --no-default-features --all-targets`、`git diff --check` は成功し、no-default の5件の warning は従来箇所に限られる。全 suite と gateway は S16 で実施する。Interface for next step は `ensure_local_approval_with_activity(..., Option<&ActivityScope>)`、汎用 `tool_activity_scope` / `finish_tool_activity` と Git scope の ordered state pattern である。CHANGES.md impact は no（operation coverage の途中であり、利用者向け記録は S16 でまとめる）。Remaining は S12〜S16 の job、delegated/integration tool、残る lifecycle、docs、全 gate、macOS 実機確認である。

- 2026-09-15: S11 independent review: approved。`gpt-6-astra` は base `878da6d` に対する immutable head `7b92c75`、full binary diff SHA-256 `286abe94a155878e22f2c08699803354690ea075f5fdeafc886e4db51d646db8` を確認した。Git の暗黙 remote を `origin` と誤表示する初回 minor を、実 argv が明示する `push --set-upstream` の既定だけ `origin`、それ以外の暗黙 pull/push は空 summary とする修正と回帰で解消し、残る指摘なしで approve した。
- 2026-09-15: S12: done。Changed は `src/activity/contract.rs` / `src/activity/render.rs`（`execute` / `start_command` / `stop_job` operation と固定 `cancellation` reason summary）、`src/mcp.rs`（sandbox job の共有 scope、worker outcome、単一 lock の terminal 直列化、stop 接続）、本イシュー記録。`JobCompletion` mutex を自然完了と停止の直列化点とし、lock 順序は completion → scope に固定した。自然完了は同じ lock 内で既存 cached result / completion time と terminal を確定する。stop は job 取得後に同じ lock 内で未確定の元操作を `reason=stop_requested` の cancelled にしてから abort し、自然完了が先なら元 terminal を変えない。session stop と2時間 lifetime は raw error から推測せず、それぞれ `reason=session_stopped` / `reason=timeout` を worker select の typed outcome で選ぶ。foreground から job へ移る場合と `start_command` の job ID 返却は scope を `JobCompletion` に保持し、返却だけでは terminal を発行しない。`stop_job` 自身は元 job と別 operation ID の通常 scope である。activity queue 投入は lock 内でも同期・非待機で、socket I/O と await は行わない。既存 job response、evidence、poll、TTL、session ownership は維持した。
  private outer test environment で `cargo test --bin temote-mcp activity_job -- --test-threads=1` は5 passed / 0 failed / 778 filtered。Yolo test session 上の実 command worker による foreground success/nonzero failure、background job 返却後の非 terminal と stop、completion lock を先取りする barrier による natural-first / stop-first、oneshot session stop、短縮 test lifetime を確認した。元操作の terminal は各経路で1件、failure は固定 `error=child_failed`、cancel は3つの固定 reason だけである。既存 stop ownership は1 passed、concurrent poll/stop と double stop は2 passed。`cargo test --lib activity::contract -- --test-threads=1` は18 passed / 0 failed / 88 filtered、`cargo test --lib activity::render -- --test-threads=1` は3 passed / 0 failed / 103 filtered。`cargo fmt --all -- --check`、`cargo clippy --all-targets -- -D warnings`、`cargo check --no-default-features --all-targets`、`git diff --check` は成功し、no-default の5件の warning は従来箇所に限られる。全 suite と gateway は S16で実施する。Interface for next step は `JobCompletion` の activity/terminal 直列化、`JobActivityOutcome`、`finish_job_completion` / `cancel_pending_job_activity` と test 用 deadline/stop future 注入であり、S13 は同じ手順を local agent/dev tool worker に適用できる。CHANGES.md impact は no（operation coverage の途中であり、利用者向け記録は S16でまとめる）。Remaining は S13〜S16 の delegated/integration tool、残る lifecycle、docs、全 gate、macOS実機確認である。

- 2026-09-15: S12 independent review: approved。`gpt-6-astra` は base `7b92c75` に対する immutable head `a24a400`、full binary diff SHA-256 `e151e0c8424c8a7c0848e16a117042d2cb82600bf2ce5b890b9466cc5935683f` を3 passと follow-up確認で approve した。初回の fixture-only minor は cancellation test の `sh -c sleep 30` が shell abort 後に grandchild を残し得る点で、3件を direct `sleep 30` に変更し対象5 testsを再実行した。検証記録も Yolo test session 上の command worker と正確に記し、残る指摘はない。
- 2026-09-15: S13: done。Changed は `src/activity/contract.rs` / `src/activity/render.rs`（`local_agent_run` / `dev_tool_run` operation）、`src/mcp.rs`（delegated tool の approval/scope、worker outcome と S12 terminal 直列化の適用、fake executable fixture）、本イシュー記録。既存 preparation、引数、sandbox profile、起動 argv、permission policy、MCP response、job/evidence は変更していない。既知 session で scope を作り、実際に approval を要求する場合だけ同一 ID で `waiting_approval`、allow/revalidation 後の worker 開始時に `running` とする。deny は running を出さず固定 `error=approval_denied` の failed、approval transport/preparation/revalidation error は outer 固定 `operation_failed`、child result は固定 completed / `error=child_failed` とする。foreground から job へ移る場合は worker と `JobCompletion` が同じ scope を保持し、job ID 返却では terminal にしない。stop/session stop/lifetime は S12 の単一 completion lock と `stop_requested` / `session_stopped` / `timeout` をそのまま使う。
  private outer test environment の Linux で `cargo test --bin temote-mcp activity_delegated_job -- --test-threads=1` は4 passed / 0 failed / 783 filtered。fake `codex` / `cargo` を既存 bounded local-agent/developer-tool profileで実行し、両 worker の agent-mode success、ask deny、短縮 lifetime timeout、job返却後の非 terminal と stop を確認した。task prompt、agent stdout、developer-tool stdout の sentinel はMCP result/approval入力に意図どおり存在しても activity update には含まれない。deny は `started → waiting_approval → failed`、agent-mode success は `started → running → completed`、timeout/stop は各1件の固定 cancelled である。S12 `activity_job` 5件と既存 local-agent denial 1件も回帰成功した。`cargo test --lib activity::contract -- --test-threads=1` は18 passed / 0 failed / 88 filtered、`cargo test --lib activity::render -- --test-threads=1` は3 passed / 0 failed / 103 filtered。`cargo fmt --all -- --check`、`cargo clippy --all-targets -- -D warnings`、`cargo check --no-default-features --all-targets`、`git diff --check` は成功し、no-default の5件の warning は従来箇所に限られる。全 suite、gateway、macOS実機は S16で実施する。Interface for next step は coverage 対象 operation enum、汎用 `tool_activity_scope` / `finish_tool_activity_on_error` と delegated worker に適用済みの `finish_job_completion` / `cancel_pending_job_activity` であり、S14 は未計測の同期 tool/integration の外側 call scopeだけを追加できる。CHANGES.md impact は no（operation coverage の途中であり、利用者向け記録は S16でまとめる）。Remaining は S14〜S16 の同期/integration coverage、残る lifecycle/upgrade、docs/CHANGES、全 gate、macOS実機確認である。
- 2026-09-15: S13 independent review: approved。`gpt-6-astra` は base `a24a400` に対する immutable head `7621faf51ad7ef247c659b3ad9c36637b80e7b50`、full binary diff SHA-256 `8c06f71d35b1a34b0ab2a6b47983712ebf4bea4cdb79836a0022058b71eeae92` を3 passで確認し、指摘なしで approve した。
- 2026-09-15: S14: done。Changed は `src/activity/contract.rs` / `src/activity/render.rs`（coverage 表の残る固定 operation と `result=accepted` の typed summary）、`src/mcp.rs`（全 advertised tool の対象・owner・成功意味・fixture の中央表、既知 session 確定後の共通 scope、call/worker terminal owner、Codex acceptance、approval 接続）、`src/apply_patch.rs` / `src/onepassword_mcp.rs`（既存 approval policy を保つ optional scope 接続）、本イシュー記録。47 advertised tool は、MCP call owner 38件、job worker owner 4件、supervisor owner 3件、明示除外2件に重複なく分類した。`execute` / `start_command` / `local_agent_run` / `dev_tool_run` は成功した受付結果で terminal にせず worker が確定し、Codex start/control の外側 call 成功だけは固定 `result=accepted` で completed とする。1Password と kintone は外側 call にだけ scope を作り、child の引数・結果・credential を summary に入れない。operation 固有の入力検証前に started、実際に中央 policy が approval を要求するときだけ waiting、allow 後の実処理前に running、明示 deny は固定 `approval_denied`、その他の error は固定 `operation_failed` とし、既存 permission policy、tool response、sandbox/network/path、public exposure は変更していない。apply_patch の既存 partial-failure response は維持しつつ activity だけを failed とする。1Password/kintone の child `isError=true` と command `exit_code != 0` も response本文を変えず固定 `child_failed` とし、payload は activity に複写しない。
  private outer test environment の Linux で `cargo test --bin temote-mcp activity_coverage -- --test-threads=1` は4 passed / 0 failed / 787 filtered。47 tool の表との完全一致・一意性・owner/fixture、全42 dispatcher row の operation/terminal owner、call success、worker non-terminal、Codex accepted、approval allow/deny順序、terminal一回性、integration の埋め込み失敗と original response 不変、raw error/result sentinel非混入を確認した。`cargo test --test activity_e2e activity_file_roundtrip` は1 passed / 0 failedで共通 dispatcher scope の実 supervisor 経路を回帰確認した。`cargo test --bin temote-mcp activity_approval` は4 passed / 0 failed / 786 filtered、`cargo test --bin temote-mcp apply_patch_` は9 passed / 0 failed / 781 filtered、1Password child env scrub は1 passed / 0 failed / 789 filtered、`cargo test activity::contract::tests` は18 passed / 0 failed / 88 filtered。`cargo fmt --all -- --check`、`cargo clippy --all-targets -- -D warnings`、`cargo check --no-default-features --all-targets`、`git diff --check` は成功し、no-default の5 warning は従来箇所に限られる。全 suite、gateway、macOS実機は S16で実施する。Interface for next step は `ACTIVITY_TOOL_COVERAGE` / `ACTIVITY_NON_DISPATCH_COVERAGE`、`ActivityResult::Accepted`、共通 `finish_covered_tool_activity`、approval の optional scope であり、S15 は supervisor-owned lifecycle/permission/upgrade を dispatcher と重複せず追加できる。CHANGES.md impact は no（利用者向け履歴はS16でまとめる）。Remaining は S15〜S16 の lifecycle/upgrade、docs/CHANGES、全gate、macOS実機確認である。

- 2026-09-15: S14 independent review: approved。`gpt-6-astra` は base `7621faf51ad7ef247c659b3ad9c36637b80e7b50` に対する immutable head `f42c12938cab8c532d999ef9c4447ed6feaaae4f`、full binary diff SHA-256 `359a318a13f95a1b4619ba71c16bfeda37dfa63ba02e1cfda81f9f4d9f6bebdb` を確認した。初回 P2 の integration 埋め込み失敗分類を、1Password/kintone MCP の `isError=true` と service-account/cli-kintone の非零 `exit_code` だけを typed boundary で固定 `child_failed` にする修正で解消し、元 response 不変・payload 非複写の回帰確認後、残る指摘なしで approve した。
- 2026-09-15: S15: done。Changed は `src/activity/contract.rs` / `src/activity/render.rs`（残る supervisor lifecycle operation と固定 upgrade phase summary）、`src/activity/scope.rs`（既存 durable UUID を使う scope constructor）、`src/approvals.rs` / `src/activity_runtime.rs`（private bounded instance bind と exact nonce ingress）、`src/supervisor.rs` / `src/session_control.rs`（permission/restart/crash/auto-restart/forget の supervisor-owned scope）、`src/http.rs` / `src/upgrade_coordinator.rs` / `src/upgrade_transaction.rs`（approval から durable coordinator phase までの upgrade scope）、本イシュー記録。instance UUID は runtime socket の1秒上限 bind で scope 作成前に取得し、emitter lifetime と queued frame に一度だけ固定する。`ActivityEmitter::try_emit` と scope/completion lock は socket I/O を行わず、旧 frame は再 bind しない。sink は legacy `id` / `started_at` / `process_id` と nonce の完全一致を要求し、missing/old nonce は破棄する。nonce、event、broker generation は disk/session metadata/upgrade plan に保存せず、tombstone も追加していない。manual restart は外側 `session_restart` だけを発行し、内部 stop/start は発行せず、新 runtime は別 instance UUID を得る。crash は検出直後に固定 clock で `started -> failed`、duration 0、`runtime_unavailable` とし、auto-restart attempt は別 scope とする。permission mode/allow/revoke、restart policy、forget は既存 validation・path・policy・return behaviorを維持する。
  remote upgrade は response-flush COMMIT 前に `started` / 実 approval時の `waiting_approval` だけを発行する。既存 transaction UUID を operation ID として再利用し、initiating session の非secretな legacy tupleだけを durable transaction に保持する。coordinator は各 transaction write の成功後にだけ fixed `phase=*` / terminal updateを既存256件 queueへ `try_send` し、delivery failureを upgrade結果へ返さない。worker はこの original tuple と現在 metadata の完全一致後にだけ current nonceを bindするため、handoff/recreate後の instanceへ旧 operationを付け替えず、旧 generationに成功 terminalを補完しない。
  private outer環境で `cargo test --bin temote-mcp activity_lifecycle -- --test-threads=1` は3 passed、`activity_ingress` は9 passed、`activity_producer` は7 passed、`coordinator_` は6 passed、`upgrade_activity_` は1 passed、全 `upgrade_` filter は69 passed、`cargo test --lib activity::contract` は18 passed、実 supervisor/MCP の `cargo test --test activity_e2e activity_file_roundtrip` は1 passed。`cargo test --test upgrade_reconnect_e2e -- --ignored --test-threads=1` は隔離 fixture で1 passed / 0 ignoredとなり、実 HTTP response flush、detached coordinator、same-PID handoff、2 session restore、ingress reconnectを確認した。決定的回帰は `activity_ingress_nonce_fences_same_tuple_after_forget_and_recreate`、`activity_ingress_missing_nonce_never_matches_a_tokenized_sink`、`activity_producer_is_ordered_and_captures_expected_instance_once`、`activity_producer_upgrade_terminal_never_reaches_recreated_instance`、`activity_lifecycle_mutations_restart_and_forget_have_single_owners`、`activity_lifecycle_crash_is_zero_duration_and_auto_restart_is_new_instance`、coordinator の persisted-state hook assertion である。`cargo fmt --all -- --check`、`cargo clippy --all-targets -- -D warnings`、`cargo check --no-default-features --all-targets`、`git diff --check` は成功した。no-default の12 dead-code warning は networkを外した既存 upgrade/approval/profile/session-control経路で、S15の upgrade producerは feature gateにより追加 warningを残さない。全 suite、gateway、英日docs、CHANGES、macOS実機はS16で実施する。Interface for next step は nonce-bound `ActivityExpectedSession`、bounded `ActivityBind`、supervisor lifecycle operation、`UpgradeCoordinatorExecutor::activity_state_persisted`、optional `UpgradeTransaction::activity_session` legacy tuple、fixed `ActivityUpgradePhase`である。CHANGES.md impact は yes（利用者向けactivity/lifecycle/upgrade変更をS16で記録）。Remaining はS16のdocs/CHANGES/全受け入れ照合/全gate/対応OS確認とS15 independent reviewである。
