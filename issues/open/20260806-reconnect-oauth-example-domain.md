...
このイシューは [20260806-purge-example-domain-history.md](20260806-purge-example-domain-history.md) の完了を前提とする。README/`.env.example` が新ドメインに更新されてから着手する。
...
- git 履歴・working tree からの旧ドメイン除去（[20260806-purge-example-domain-history.md](20260806-purge-example-domain-history.md) で扱う）。
...
- [ ] `curl -i -X POST https://temotemcp.example.com/mcp ...`（未認証）が `401` と `WWW-Authenticate` ヘッダーを返す。
- [ ] `curl -i https://temotemcp.example.com/.well-known/oauth-authorization-server` が `200` を返す。
- [ ] `curl -i https://temotemcp.example.com/.well-known/oauth-protected-resource` が `200` を返す。
...
- [20260806-purge-example-domain-history.md](20260806-purge-example-domain-history.md) の完了後に着手する。
