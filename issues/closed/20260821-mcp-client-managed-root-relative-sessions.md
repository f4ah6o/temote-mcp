...
The next Ubuntu run (`32428395105`) made the host restriction explicit: `bwrap: loopback: Failed RTM_NEWADDR: Operation not permitted`. This is the exact AppArmor/user-namespace failure detected by `temote-mcp doctor`. CI now applies the repository's documented `bwrap-userns-restrict` AppArmor profile before running the namespace probe; it does not disable or bypass the Linux sandbox.
