...
Completed through pull request 10 on 2026-09-04.

- A per-invocation Unix-domain-socket broker, exact locator allowlist, owner and UID checks, and cleanup were implemented.
- The raw `OP_SERVICE_ACCOUNT_TOKEN` remains outside the child environment and broker output is redacted without exposing resolved values.
- Concurrent invocation isolation, denied locators, truncated-output redaction, non-Linux fail-closed behavior, and the existing service-account path were covered by tests.
- Rust formatting, tests, clippy, no-default-feature checks, gateway tests, and diff validation passed; the pull request was merged.

The local issue was moved from `open/` to `done/` after the merged pull request and successful CI supplied both the implementation and completion evidence.
