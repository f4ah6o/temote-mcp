...
2. A **self-hosted Cloudflare Access application** that directly protects the
   ...
   `localmcp-direct`, the public hostname `localmcp.f12o.com`, and leave the
...
`localmcp.f12o.com` when ChatGPT connects directly to that hostname. Such an
...
of the self-hosted application that protects `localmcp.f12o.com`; do not
...
the Rust origin:
    ...
    curl -i -X POST https://localmcp.f12o.com/mcp \
    ...
    curl -i https://localmcp.f12o.com/.well-known/oauth-authorization-server
    curl -i https://localmcp.f12o.com/.well-known/oauth-protected-resource
...
`https://localmcp.f12o.com/mcp`; the host root is only used for OAuth
