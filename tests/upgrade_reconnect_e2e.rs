...
fn copy_source_tree(destination: &Path) {
    ...
    git.args(["ls-files", "-z", "--cached"]);
    if std::env::var_os("TEMOTE_TEST_INCLUDE_UNTRACKED_SOURCE").as_deref()
        == Some(std::ffi::OsStr::new("1"))
    {
        // Local pre-commit verification may include only untracked files under
        // the same narrow source allow-list. CI remains tracked-only by default.
        git.args(["--others", "--exclude-standard"]);
    }
    git.args([
        "--",
        "Cargo.toml",
    ...
    assert!(output.status.success(), "git source inventory failed");
    let names = output
        .stdout
        .split(|byte| *byte == 0)
    ...
    assert!(
        ...
        "source fixture exceeds {MAX_SOURCE_FILES} files"
    );
    ...
    for name in names {
        ...
        let metadata = fs::symlink_metadata(&source)
            .unwrap_or_else(|error| panic!("failed to inspect source {name}: {error}"));
        assert!(
            metadata.file_type().is_file(),
            "source fixture accepts only regular files (no submodules or symlinks): {name}"
        );
        ...
        assert!(
            ...
            "source fixture exceeds {MAX_SOURCE_BYTES} bytes"
        );
        if let Some(parent) = target.parent() {
            fs::create_dir_all(parent).expect("failed to create copied source directory");
        }
        fs::copy(&source, &target)
            .unwrap_or_else(|error| panic!("failed to copy source {name}: {error}"));
    }
}
